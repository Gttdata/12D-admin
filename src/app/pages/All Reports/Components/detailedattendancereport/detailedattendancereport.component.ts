...
@Component({
  selector: 'app-detailedattendancereport',
  templateUrl: './detailedattendancereport.component.html',
  styleUrls: ['./detailedattendancereport.component.css'],
})
export class DetailedattendancereportComponent implements OnInit {
  formTitle = ' Detailed Attendance Report ';
  dataList: any[] = [];
  exportdataList: any[] = [];
  ...
  ngOnInit(): void {
    ...
    if (sessionStorage.getItem('Classid')) {
      this.classes = sessionStorage.getItem('Classid');
    }
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.type) {
      extraQuery += " AND STATUS= '" + this.type + "'";
    }
  ...
  convertInExcel() {
    ...
    if (this.exportdataList.length > 0) {
      for (var i = 0; i < this.exportdataList.length; i++) {
        ...
        obj1['Subject Name'] = this.exportdataList[i]['SUBJECT_NAME'];
        if (this.exportdataList[i]['STATUS'] == 'P') {
          obj1['Status'] = 'Present';
        } else {
          obj1['Status'] = 'Absent';
        }
