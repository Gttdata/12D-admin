...
@Component({
  selector: 'app-prearrangeactivitydetailedreport',
  templateUrl: './prearrangeactivitydetailedreport.component.html',
  styleUrls: ['./prearrangeactivitydetailedreport.component.css'],
})
export class PrearrangeactivitydetailedreportComponent implements OnInit {
  formTitle = 'Pre Arranged Activity Head Detailed Report';
  dataList: any[] = [];
  exportdataList: any[] = [];
  ...
  getAllActivities() {
    ...
    this.api
      .getActivityHeadMaster(0, 0, 'id', 'desc', ' AND STATUS=1')
      .subscribe((data) => {
        if (data['code'] == 200) {
    ...
    if (exportInExcel == false) {
      this.api
        .getPreActivitydetailed(
          ...
          this.sortKey,
          sort,
          likeQuery + this.filterQuery + ' AND HEAD_USER_ID = 0'
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  applyFilter() {
    ...
    if (
      ...
      this.filterQuery += ' AND ACTIVITY_HEAD =' + this.activtyId;
    }
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.exportdataList.length; i++) {
      obj1['Pre Arranged Activity Head Name'] =
        this.exportdataList[i]['HEAD_NAME'];
      obj1['User Name'] = this.exportdataList[i]['USER_NAME'];
      obj1['Started Date Time'] = this.exportdataList[i]['START_DATETIME']
        ? this.datePipe.transform(
            this.exportdataList[i]['START_DATETIME'],
            'dd/MMM/yyyy HH:mm'
          )
        : '';
      obj1['Completed Percentage'] = this.exportdataList[i].COMPLETED_PERCENTAGE
        ? this.exportdataList[i].COMPLETED_PERCENTAGE + ' %'
        : '';
      obj1['Is Completed'] = this.exportdataList[i].END_DATETIME?'✓':'X'
      ...
      if (i == this.exportdataList.length - 1) {
        this.exportservice.exportExcel(
          arry1,
          'Pre Arranged Activity Head Detailed Report ' +
