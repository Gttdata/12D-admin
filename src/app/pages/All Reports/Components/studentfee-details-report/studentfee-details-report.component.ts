...
@Component({
  selector: 'app-studentfee-details-report',
  templateUrl: './studentfee-details-report.component.html',
  styleUrls: ['./studentfee-details-report.component.css'],
})
export class StudentfeeDetailsReportComponent implements OnInit {
  formTitle = ' Student Fee Details Report ';
  dataList: any[] = [];
  exportdataList: any[] = [];
  ...
  ngOnInit(): void {
    ...
    // }
    if (sessionStorage.getItem('PendingFilter')) {
      this.pendingFilter = sessionStorage.getItem('PendingFilter');
    }
    if (sessionStorage.getItem('piedivisionFilterid')) {
      this.divisionFilter = sessionStorage.getItem('piedivisionFilterid');
    }
    if (sessionStorage.getItem('pieYearFilter')) {
      this.yearFilter = sessionStorage.getItem('pieYearFilter');
    }
    if (sessionStorage.getItem('pieClassFilter')) {
      this.classFilter = sessionStorage.getItem('pieClassFilter');
    }
  }
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.pendingFilter) {
      if (this.pendingFilter == 'P') {
        extraQuery += ' AND PAID_FEE = 0 ';
      } else {
        extraQuery += ' AND PENDING_FEE = 0 ';
      }
    }
    if (this.yearFilter) {
  ...
  convertInExcel() {
    ...
    if (this.exportdataList.length > 0) {
      for (var i = 0; i < this.exportdataList.length; i++) {
        ...
        obj1['Paid Fee'] = this.exportdataList[i]['PAID_FEE'];
        obj1['Pending Fee'] = this.exportdataList[i]['PENDING_FEE'];
        obj1['Total Fee'] = this.exportdataList[i]['TOTAL_FEE'];

        arry1.push(Object.assign({}, obj1));
        if (i == this.exportdataList.length - 1) {
          this.exportservice.exportExcel(
            arry1,
            'Student Fee Details Report ' +
