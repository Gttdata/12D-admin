...
@Component({
  selector: 'app-taskcompeltionsummary',
  templateUrl: './taskcompeltionsummary.component.html',
  styleUrls: ['./taskcompeltionsummary.component.css'],
})
export class TaskcompeltionsummaryComponent implements OnInit {
  formTitle = 'Task Completion Summary Report';
  dataList: any[] = [];
  exportdataList: any[] = [];
  ...
  sortKey: string = 'DATE(ASSIGNED_DATE)';
  searchText: string = '';
  filterQuery: string = '';
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    if (reset) {
      this.pageIndex = 1;
      this.sortKey = 'DATE(ASSIGNED_DATE)';
      this.sortValue = 'desc';
    }
  ...
  sort(params: NzTableQueryParams): void {
    ...
    const sortField = (currentSort && currentSort.key) || 'DATE(ASSIGNED_DATE)';
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.exportdataList.length; i++) {
      obj1['Date'] = this.exportdataList[i]['DATE']
