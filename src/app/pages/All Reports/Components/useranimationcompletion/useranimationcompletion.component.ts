...
@Component({
  selector: 'app-useranimationcompletion',
  templateUrl: './useranimationcompletion.component.html',
  styleUrls: ['./useranimationcompletion.component.css'],
})
export class UseranimationcompletionComponent implements OnInit {
  formTitle = 'User Wise Animation Completion Report';
  dataList: any[] = [];
  exportdataList: any[] = [];
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.exportdataList.length; i++) {
      ...
      obj1['Animation Part'] =
        this.exportdataList[i]['START_DURATION'] &&
        this.exportdataList[i]['END_DURATION']
          ? this.exportdataList[i]['START_DURATION'] +
            ' - ' +
            this.exportdataList[i]['END_DURATION']
          : '';

      // if (this.exportdataList[i]['STATUS'] == 'A') {
      //   obj1['Status'] = 'Activated';
      // } else if (this.exportdataList[i]['STATUS'] == 'C') {
      //   obj1['Status'] = 'Completed';
      // }
      obj1['Started Date'] = this.exportdataList[i]['START_DATE']
        ? this.datePipe.transform(
            this.exportdataList[i]['START_DATE'],
            'dd/MMM/yyyy'
          )
        : '';
      obj1['Number Of Rewards'] = this.exportdataList[i]['NUMBER_OF_REWARD'];
      obj1['Pending Rewards'] = this.exportdataList[i]['PENDING_REWARDS'];
      obj1['Completed Rewards'] = this.exportdataList[i]['COMPLETED_REWARDS'];

      arry1.push(Object.assign({}, obj1));
      if (i == this.exportdataList.length - 1) {
        this.exportservice.exportExcel(
          arry1,
          'User Wise Animation Completion Report ' +
