...
export class CountryMasterComponent implements OnInit {
  ...
  columns: string[][] = [
    ['NAME', ' NAME'],
    ['IS_STATE_AVALIBLE', 'IS_STATE_AVALIBLE'],
    ['SEQ_NO', 'SEQ_NO'],
  ...
  add(): void {
    ...
    this.api.getALLCountry(1, 1, 'SEQ_NO', 'desc', '').subscribe(
      (data) => {
  ...
  save(addNew: boolean, websitebannerPage: NgForm): void {
    ...
    if (this.isOk) {
      ...
      if (this.drawerData.ID) {
        this.api.UpdateCountry(this.drawerData).subscribe((successCode) => {
          if (successCode.code == '200') {
            this.message.success(' Country Information Updated Successfully...', '');
            if (!addNew) this.drawerClose();
            this.isSpinning = false;
          } else {
            this.message.error(' Failed To Update Country Information...', '');
            this.isSpinning = false;
          }
        });
      } else {
        this.api
          .CreateCountry(this.drawerData)
          .subscribe((successCode) => {
            if (successCode.code == '200') {
              this.message.success(' Country Information Saved Successfully...', '');
              if (!addNew) this.drawerClose();
              else {
                this.drawerData = new CountryMaster();
                this.resetDrawer(websitebannerPage);
                this.api.getALLCountry(1, 1, '', 'desc', '').subscribe(
                  (data) => {
                    if (data['code'] == 200) {
                      if (data['count'] == 0) {
                        this.drawerData.SEQ_NO = 1;
              ...
              this.message.error(' Failed To Save Country Information...', '');
              this.isSpinning = false;
            }
          });
      }
